...
const Home = () => {
  const {
    ...
    isAiMatching, aiSkills, handleResumeUpload,
    pagination, goToPage,
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-10 bg-gradient-to-r from-blue-900/40 to-purple-900/40 border border-blue-500/30 p-8 rounded-3xl shadow-2xl relative overflow-hidden">
          <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
            <svg className="w-48 h-48 text-white" fill="currentColor" viewBox="0 0 20 20"><path d="M9 2a1 1 0 000 2h2a1 1 0 100-2H9z"/><path fillRule="evenodd" d="M4 5a2 2 0 012-2 3 3 0 003 3h2a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H6a2 2 0 01-2-2V5zm3 4a1 1 0 000 2h.01a1 1 0 100-2H7zm3 0a1 1 0 000 2h3a1 1 0 100-2h-3zm-3 4a1 1 0 100 2h.01a1 1 0 100-2H7zm3 0a1 1 0 100 2h3a1 1 0 100-2h-3z" clipRule="evenodd"/></svg>
          </div>

          <h2 className="text-2xl font-bold text-white mb-2 flex items-center">
            <span className="bg-blue-500 text-white text-xs px-2 py-1 rounded mr-3 uppercase font-black tracking-widest">AI</span>
            Resume Matcher
          </h2>
          <p className="text-blue-200 mb-6 max-w-2xl text-sm">
            Drop your PDF resume and our AI instantly extracts your skills and shows only the most relevant jobs from our database.
          </p>
          ...
          >
            ...
            {isAiMatching ? (
              ...
              <div className="flex flex-col items-center text-center">
                ...
                <p className="text-green-300 font-medium text-lg">Matched! Jobs updated below.</p>
                <p className="text-gray-400 text-sm mt-1">{uploadedFileName}</p>
                <p className="text-blue-400 text-xs mt-3 underline">Click to upload a different resume</p>
              </div>
            ) : (
              <div className="flex flex-col items-center text-center">
          ...
          {aiSkills?.length > 0 && (
            <div className="mt-6 pt-6 border-t border-blue-500/20">
              <p className="text-xs text-gray-400 mb-3 font-bold uppercase tracking-widest">Extracted Skills:</p>
              <div className="flex flex-wrap gap-2">
                {aiSkills.map(skill => (
                  <span key={skill} className="bg-blue-600/20 text-blue-300 border border-blue-500/30 px-3 py-1 rounded-full text-sm font-semibold capitalize">{skill}</span>
                ))}
              </div>
            </div>
          )}
        ...
        <div className="bg-[#1E293B] p-6 rounded-3xl mb-8 shadow-2xl border border-gray-800/50">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { label: 'What role?', name: 'role', placeholder: 'e.g. Software Engineer', type: 'text' },
              { label: 'Where?', name: 'city', placeholder: 'e.g. Bangalore or Remote', type: 'text' },
              ...
              <div key={name} className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={handleFilterChange} placeholder={placeholder}
            ...
            <div className="space-y-1.5">
              ...
              <select
                name="companyType" value={filters.companyType} onChange={handleFilterChange}
                className="w-full bg-[#0F172A] border border-gray-700/50 rounded-2xl px-4 py-3 text-white focus:outline-none focus:ring-2 focus:ring-blue-500/50 focus:border-blue-500 transition-all cursor-pointer"
              >
        ...
        {!loading && !error && (
          <div className="mb-4 flex items-center justify-between">
            <p className="text-gray-500 text-sm">
              {aiSkills?.length > 0 ? (
                <span>✨ <span className="text-white font-semibold">{jobs.length}</span> AI-matched jobs for your resume</span>
              ) : (
                <span><span className="text-white font-semibold">{pagination.total}</span> jobs found</span>
              )}
            </p>
            {!isAuthenticated && (
              <Link to="/register" className="text-sm text-blue-400 hover:text-blue-300 font-medium">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
          {loading ? (
            <div className="col-span-full flex flex-col items-center justify-center py-24 text-gray-400">
              ...
              <p className="text-xl font-medium animate-pulse">Loading jobs...</p>
            </div>
          ) : error ? (
            ...
            jobs.map((job) => {
              ...
              return (
                <div key={job._id} className="group bg-[#1E293B] border border-gray-800/50 rounded-[2rem] p-7 hover:bg-[#243147] hover:border-blue-500/30 transition-all duration-300 flex flex-col shadow-xl hover:shadow-blue-900/10">
                  {/* Card Header */}
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        {job.source && job.source !== 'Manual' && (
                          <p className="text-gray-600 text-[10px]">via {job.source}</p>
                        )}
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      {job.companyType && (
                        <span className={`text-[9px] px-2.5 py-1 rounded-full uppercase font-black tracking-widest border ${typeColor}`}>
                          {job.companyType}
                        </span>
                      )}
                      {isAuthenticated && (
                  ...
                  <div className="flex flex-wrap gap-2 my-4">
                    ...
                    {job.salary && job.salary !== 'Not Disclosed' && (
                      <span className="text-xs bg-gray-900/50 text-gray-400 px-2.5 py-1 rounded-lg border border-gray-800">💰 {job.salary}</span>
                    )}
                  </div>
            ...
            <div className="col-span-full flex flex-col items-center justify-center py-24 text-gray-600">
              ...
              <p className="text-sm opacity-60 mt-1">Try adjusting your filters or check back later.</p>
            </div>
          )}
        </div>

        {/* Pagination */}
        {!loading && !error && pagination.totalPages > 1 && aiSkills?.length === 0 && (
